...
export default function DynamicTable({ tabName, title, columns: propColumns }: DynamicTableProps) {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // Get headers and data from API
            const apiHeaders: string[] = result.headers || [];
            const apiData: string[][] = result.data || [];
            ...
            // Transform array of arrays to objects based on headers
            const formatted = apiData.map((row: string[]) => {
                return convertRowToObject(row, apiHeaders);
            });
            setData(formatted);
        } catch (err) {
    ...
    // Get row index in original data (for delete)
    const getRowIndex = (itemId: string): number => {
        return data.findIndex(d => d[headers[0]?.toLowerCase().replace(/\s+/g, '_')] === itemId);
    };
    ...
    // Get ID column key (usually first column)
    const getIdKey = (): string => {
        if (headers.length === 0) return 'id';
        return headers[0].toLowerCase().replace(/\s+/g, '_');
    };
                                    ...
                                    paginatedData.map((item) => {
                                        ...
                                        return (
                                            <tr key={item[idKey]} className="hover:bg-gray-50 transition-colors">
                                                {/* ID Column */}
                                                <td className="px-6 py-4 text-sm text-gray-800 font-medium">{item[idKey]}</td>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex space-x-2">
                                                        ...
                                                        <button onClick={() => handleDelete(item[idKey])} className="p-2 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
